...
- **`buckets[]`** — `day` (RFC 3339; the start of the bucket's calendar day in the status page's configured display timezone — UTC midnight only for pages without one, so resolve the instant in that zone rather than truncating the string), the `bigint` counts `count` / `ok` / `degraded` / `error`, a resolved `status` (`ComponentDayStatus`: operational / degraded / down / maintenance / empty), and an optional `impact` (`PageComponentImpact`) for the worst status-report impact that day. Monitor-backed components carry real counts; static components have empty buckets.
